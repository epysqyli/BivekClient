  ...
  return (
    <div className='flex justify-between items-center border dark:border-none bg-white dark:bg-slate-500 shadow-sm shadow-slate-300 dark:shadow-none rounded pl-3'>
      ...
      <Link href={`edit-articles/${article.id}`}>
        <div className='flex items-center self-stretch px-5 bg-slate-50 dark:bg-slate-400 hover:bg-slate-100 transition-colors rounded-br rounded-tr cursor-pointer group'>
          <Edit className='text-slate-500 dark:text-white group-hover:text-slate-600 dark:group-hover:text-slate-200 group-active:text-slate-900 dark:group-active:text-slate-300' />
        </div>
      </Link>
    </div>
  );
